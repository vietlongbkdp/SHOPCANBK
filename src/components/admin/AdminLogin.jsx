...
export default function AdminLogin({ onSuccess }) {
  ...
  return (
    <Box sx={{
      minHeight: '100vh', background: 'linear-gradient(135deg, #1565c0, #00b0ff)',
      display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2,
    }}>
      <Paper sx={{ width: '100%', maxWidth: 380, borderRadius: 2, overflow: 'hidden' }}>
        {/* Header */}
        <Box sx={{ background: 'linear-gradient(135deg, #1565c0, #00b0ff)', p: 3, textAlign: 'center', color: 'white' }}>
          ...
          <form onSubmit={handleSubmit}>
            <Stack spacing={2}>
              ...
              <Button type="submit" variant="contained" fullWidth disabled={loading}
                sx={{ background: '#1565c0', fontWeight: 700, py: 1.2, '&:hover': { background: '#8e0000' } }}>
