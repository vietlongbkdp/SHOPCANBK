...
import {
  ...
  List, ListItem, ListItemText, Divider, Collapse, Badge,
} from '@mui/material';
        ...
        <Toolbar sx={{ minHeight: { xs: 48, md: 52 }, px: { xs: 1, md: 2 }, gap: 0.5 }}>
                ...
                item.children ? (
                  <Box key={item.label}>
                    ...
                    <Menu anchorEl={productsAnchor} open={Boolean(productsAnchor)} onClose={() => setProductsAnchor(null)}>
                      <MenuItem onClick={() => { onNavigate('products'); setProductsAnchor(null); }}>Tất Cả Sản Phẩm</MenuItem>
                      <Divider />
                      {item.children.map((c) => (
                        <MenuItem key={c.label} onClick={() => { onNavigate('products'); setProductsAnchor(null); }}>{c.label}</MenuItem>
                      ))}
                    </Menu>
                  </Box>
                ) : (
          ...
          <Stack direction="row" spacing={0.5} alignItems="center">
            ...
            <Box sx={{
              ...
              background: 'rgba(255,255,255,0.15)', borderRadius: 1, px: 1.5, py: 0.3,
              border: '1px solid rgba(255,255,255,0.3)',
              width: { xs: 130, sm: 180, md: 210 },
              '&:focus-within': { background: 'rgba(255,255,255,0.25)' },
            }}>
              <InputBase placeholder="Tìm kiếm..." value={searchTerm} onChange={(e) => onSearch(e.target.value)}
            ...
            {/* Cart */}
            <IconButton onClick={onOpenCart} sx={{ color: 'white' }}>
              <Badge badgeContent={totalCount} color="error" sx={{ '& .MuiBadge-badge': { fontSize: 10, minWidth: 16, height: 16 } }}>
